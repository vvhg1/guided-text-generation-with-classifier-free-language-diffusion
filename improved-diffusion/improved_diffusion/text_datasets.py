...
def load_data_text(
    *, data_dir, batch_size, image_size, class_cond=False, deterministic=False, data_args=None, 
    ...
    if task_mode == 'roc' or task_mode == 'roc-aug' or task_mode == 'roc-free':
        print('hello loading roc, roc-aug or roc-free. ')
        ...
        print('hello loading simple-wiki. ')
        training_data, model = get_corpus_rocstory(data_args, model, image_size,
                                            padding_mode=padding_mode, split=split,
                                            load_vocab=load_vocab)
        ...
        print('hello loading roc story e2e. ')
        training_data, model = get_corpus_rocstory(data_args, model, image_size,
                                            padding_mode=padding_mode, split=split,
                                            load_vocab=load_vocab)

    elif task_mode == 'book':
        print('hello loading roc story book. ')
        tokenizer = AutoTokenizer.from_pretrained('bert-base-uncased')
        training_data, model = get_corpus_book(data_args, tokenizer, model, image_size,
                                              padding_mode=padding_mode, split=split,)

    if data_args.modality in ['roc-free', 'roc-aug', 'roc', 'book', 'yelp', 'commonGen', 'commonGen-aug'] and data_args.cache_mode=='no':
...
def helper_tokenize_stream(sentence_lst, vocab_dict, model, seqlen, data_args, padding_mode, ):
    ...
    sentence_lst, emotion_lst = zip(*sentence_lst)
    raw_datasets = Dataset2.from_dict({'text':sentence_lst,'emotion':emotion_lst})
    print(raw_datasets)
    print(f"RAM used: {psutil.Process().memory_info().rss / (1024 * 1024):.2f} MB")
...
def get_corpus_rocstory(data_args, model, image_size, padding_mode='block',
    ...
    if data_args.experiment_mode == 'lm':
        if data_args.modality == 'roc-free':
            print('loading dataset from ROCStory with keys for classifier free guidance')
            nlp = English()
            tokenizer = nlp.tokenizer
            sentence_lst = []
            emotion_lst = []
            #TODO:make arg, this is the flag for splitting the story into sentences
            ...
            if split == 'train':
                ...
                if splitsentences:
                    path_emotion = f'{data_args.roc_train}/roc_train_emotion_split_True_scores.json'
                else:    
                    path_emotion = f'{data_args.roc_train}/roc_train_emotion_split_False_scores.json'
            elif split == 'valid':
                print('loading form the VALID set')
                path = f'{data_args.roc_train}/roc_valid.json'
                path_emotion = f'{data_args.roc_train}/roc_valid_emotion_split_False_scores.json'
            else:
            ...
            with open(path, 'r') as roc_reader:
                for row in roc_reader:
                    ...
                    if splitsentences:
                        # split sentences
                        split_sentences = re.split("(?<=[.!?])\s+", sentences)
                        for s in split_sentences:
                            word_lst = [x.text for x in tokenizer(s)]
                            sentence_lst.append(word_lst)
                    else:
                        word_lst = [x.text for x in tokenizer(sentences)]
                        sentence_lst.append(word_lst)
            with open(path_emotion, 'r') as rocfree_reader:
                for row in rocfree_reader:
                    emotions = json.loads(row)
                    emotion_lst.append(emotions)
                    
            complete_list = list(zip(sentence_lst, emotion_lst))
            sentence_lst = complete_list
    ...
    if data_args.experiment_mode == 'lm' and data_args.modality in ['roc-free','roc-aug', 'roc', 'yelp', 'commonGen', 'commonGen-aug'] \
    ...
    def __getitem__(self, idx):
        ...
        with torch.no_grad():
            ...
            if self.model_arch == 'conv-unet':
                ...
                out_dict = {}
                out_dict['input_ids'] = np.array(self.text_datasets['train'][idx]['input_ids'])
                out_dict['emotion'] = np.array(self.text_datasets['train'][idx]['emotion'])
                # out_dict['mapping_func'] = self.mapping_func
                if self.data_args.experiment_mode == 'conditional_gen':
                    out_dict['src_ids'] = np.array(self.text_datasets['train'][idx]['src_ids'])
                    out_dict['src_mask'] = np.array(self.text_datasets['train'][idx]['src_mask'])
