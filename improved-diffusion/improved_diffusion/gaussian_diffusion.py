...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        if self.model_arch == 'conv-unet' or self.model_arch == '1d-unet':
            ...
            B, C = x.size(0), x.size(-1)
        assert t.shape == (B,)
        model_output = model(x, self._scale_timesteps(t),emotion)
    ...
    def ddim_sample(
        ...
        ###########! set guidance_scale to zero if t is smaller than param ###########
        ##########################! push eps w guidance_scale ########################
        guidance_scale = 1.2                                                        #!
        if t[0] < 10:                                                                  #!
            guidance_scale = 0                                                      #!
    ...
    def training_losses_e2e(self, model, x_start, t, model_kwargs=None, noise=None):
        ...
        if self.loss_type == LossType.E2E_KL:
            ...
            # print(x_t.shape)
            model_output = model(x_t, self._scale_timesteps(t), emotion, **model_kwargs)
