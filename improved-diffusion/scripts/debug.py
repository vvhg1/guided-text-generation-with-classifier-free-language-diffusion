...
def main():
    ...
    while len(all_images) * args.batch_size < args.num_samples:
        ...
        def get_noise_from_final(final_gold):
            final_gold = final_gold.to(dist_util.dev())
        ...
        # recon_ = denoised_fn_round(args, model3.to(dist_util.dev()), naive_recon, t_batch)
        # mse = ((recon_ - batch.to(recon_.device)) ** 2).mean()
        # print(mse, 'naive_mst')
        # # ppl:
        # nll = compute_logp(args, model3.to(dist_util.dev()), naive_recon, cond['input_ids'].to(recon_.device))
        # print(nll.mean())
        ...
        for sample in diffusion.p_debug_loop_progressive(
                ...
                # denoised_fn=partial(denoised_fn_round,args,  model3.to(dist_util.dev())), #EDIT
